...
int tumgrd_db_open(struct tumgrd_db *db, const char *path) {
  ...
  if (ensure_parent_dir(db->path) != 0) {
  ...
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "sqlite3_open_v2", err);
    if (db->conn) {
...
int tumgrd_db_upsert_node(struct tumgrd_db *db, const struct tumgrd_node *node) {
  ...
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "sqlite3_prepare_v2(upsert)", err);
    return -1;
  }

  err = bind_required_text(stmt, 1, n.uid);
  err = err == SQLITE_OK ? bind_text_or_null(stmt, 2, nonempty_or_null(n.description)) : err;
  err = err == SQLITE_OK ? bind_text_or_null(stmt, 3, nonempty_or_null(n.client_comment)) : err;
  err = err == SQLITE_OK ? sqlite3_bind_int64(stmt, 4, (sqlite3_int64) n.created_at) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 5, n.server_host) : err;
  err = err == SQLITE_OK ? sqlite3_bind_int(stmt, 6, n.server_port) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 7, n.psk) : err;
  ...
  err = err == SQLITE_OK ? bind_text_or_null(stmt, 10, nonempty_or_null(n.ip_check_url)) : err;
  err = err == SQLITE_OK ? bind_text_or_null(stmt, 11, nonempty_or_null(n.ip_version)) : err;
  err = err == SQLITE_OK ? bind_text_or_null(stmt, 12, nonempty_or_null(n.current_ip)) : err;
  err = err == SQLITE_OK ? sqlite3_bind_int64(stmt, 13, (sqlite3_int64) n.last_updated) : err;
  err = err == SQLITE_OK ? bind_text_or_null(stmt, 14, nonempty_or_null(n.status)) : err;

  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "bind(upsert)", err);
  ...
  if (err != SQLITE_DONE) {
    sqlite_log_error(db->conn, "sqlite3_step(upsert)", err);
    sqlite3_finalize(stmt);
    return -1;
  }
...
int tumgrd_db_get_node(struct tumgrd_db *db, const char *server_host, int server_port, const char *uid, const char *ip_version,
  ...
  err = sqlite3_prepare_v2(db->conn, sql, -1, &stmt, NULL);
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "sqlite3_prepare_v2(get)", err);
    return -1;
  }

  err = bind_required_text(stmt, 1, server_host);
  err = err == SQLITE_OK ? sqlite3_bind_int(stmt, 2, server_port) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 3, uid) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 4, ip_version) : err;

  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "bind(get)", err);
    sqlite3_finalize(stmt);
    return -1;
  }
  ...
  sqlite_log_error(db->conn, "sqlite3_step(get)", err);
  sqlite3_finalize(stmt);
  return -1;
}
...
int tumgrd_db_delete_node(struct tumgrd_db *db, const char *server_host, int server_port, const char *uid,
  ...
  err = sqlite3_prepare_v2(db->conn, sql, -1, &stmt, NULL);
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "sqlite3_prepare_v2(delete)", err);
    return -1;
  }
  ...
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "bind(delete)", err);
    sqlite3_finalize(stmt);
    return -1;
  }
  ...
  err = sqlite3_step(stmt);
  if (err != SQLITE_DONE) {
    sqlite_log_error(db->conn, "sqlite3_step(delete)", err);
    sqlite3_finalize(stmt);
    return -1;
  }
...
int tumgrd_db_list_nodes(struct tumgrd_db *db, struct tumgrd_node **nodes, size_t *count) {
  ...
  *count = 0;
  ...
  err = sqlite3_prepare_v2(db->conn, sql, -1, &stmt, NULL);
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "sqlite3_prepare_v2(list)", err);
    return -1;
  }
  ...
  for (;;) {
    ...
    sqlite_log_error(db->conn, "sqlite3_step(list)", err);
    sqlite3_finalize(stmt);
    free(arr);
    return -1;
  }
...
int tumgrd_db_update_runtime(struct tumgrd_db *db, const char *server_host, int server_port, const char *uid,
  ...
  err = sqlite3_prepare_v2(db->conn, sql, -1, &stmt, NULL);
  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "sqlite3_prepare_v2(update_runtime)", err);
    return -1;
  }

  err = bind_text_or_null(stmt, 1, nonempty_or_null(current_ip));
  err = err == SQLITE_OK ? bind_required_text(stmt, 2, status) : err;
  err = err == SQLITE_OK ? sqlite3_bind_int64(stmt, 3, (sqlite3_int64) last_updated) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 4, server_host) : err;
  err = err == SQLITE_OK ? sqlite3_bind_int(stmt, 5, server_port) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 6, uid) : err;
  err = err == SQLITE_OK ? bind_required_text(stmt, 7, ip_version) : err;

  if (err != SQLITE_OK) {
    sqlite_log_error(db->conn, "bind(update_runtime)", err);
    sqlite3_finalize(stmt);
    return -1;
  }
  ...
  err = sqlite3_step(stmt);
  if (err != SQLITE_DONE) {
    sqlite_log_error(db->conn, "sqlite3_step(update_runtime)", err);
    sqlite3_finalize(stmt);
    return -1;
  }
